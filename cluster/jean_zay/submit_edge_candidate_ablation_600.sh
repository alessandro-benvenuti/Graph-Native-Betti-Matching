#!/usr/bin/env bash
# Submit four edge-candidate ablations and dependent MRI specializations.
set -euo pipefail
...
dry_run="${GNBM_ABLATION_DRY_RUN:-0}"
...
pretrain_runs=(
  pretrain_mixed_nodece_edgefocal_matchedonly_seed364505
  pretrain_mixed_nodefocal_edgefocal_matchedonly_seed364505
  pretrain_mixed_nodece_edgece_unmatched_seed364505
  pretrain_mixed_nodefocal_edgece_unmatched_seed364505
)

finetune_runs=(
  finetune_mri600_nodece_edgefocal_matchedonly_seed364505
  finetune_mri600_nodefocal_edgefocal_matchedonly_seed364505
  finetune_mri600_nodece_edgece_unmatched_seed364505
  finetune_mri600_nodefocal_edgece_unmatched_seed364505
)

for index in "${!recipes[@]}"; do
  recipe="${recipes[$index]}"
  for stage in pretrain finetune; do
    config="$repo_dir/configs/experiments/edge_candidate_ablation_600/${stage}_${recipe}.yaml"
    if [[ ! -f "$config" ]]; then
...
export WANDB_RUN_GROUP="${WANDB_RUN_GROUP:-edge-candidate-ablation-600-seed364505}"
...
for index in "${!recipes[@]}"; do
  recipe="${recipes[$index]}"
  pretrain_config="configs/experiments/edge_candidate_ablation_600/pretrain_${recipe}.yaml"
  finetune_config="configs/experiments/edge_candidate_ablation_600/finetune_${recipe}.yaml"
