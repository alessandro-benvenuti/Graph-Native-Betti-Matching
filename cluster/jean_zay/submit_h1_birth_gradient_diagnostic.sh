#!/usr/bin/env bash
# Submit the frozen-checkpoint H1 birth-edge gradient diagnosis.
set -euo pipefail

repo_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
default_experiment="/lustre/fsn1/projects/rech/vnc/upz73jr/checkpoints/gnbm-node-focal-topology-overfit-a100"
experiment="${1:-${GNBM_MECH_OUTPUT:-$default_experiment}}"
output="${GNBM_H1_DIAGNOSTIC_OUTPUT:-$experiment/h1-false-birth-diagnostic}"
qos="${GNBM_H1_DIAGNOSTIC_QOS:-qos_gpu_a100-dev}"
walltime="${GNBM_H1_DIAGNOSTIC_WALLTIME:-02:00:00}"

if ! command -v sbatch >/dev/null 2>&1; then
  echo "sbatch is unavailable; run this on a Jean Zay login node." >&2
  exit 2
fi
for name in WORK SYNTHETIC_MRI_DATASET; do
  if [[ -z "${!name:-}" ]]; then
    echo "$name is not set." >&2
    exit 2
  fi
done
if [[ ! -f "$experiment/experiment-complete" ]]; then
  echo "The topology-overfit experiment is incomplete: $experiment" >&2
  exit 2
fi
if [[ -f "$output/diagnostic-complete" ]]; then
  echo "H1 gradient diagnostic is already complete: $output" >&2
  exit 2
fi
case "$qos" in
  qos_gpu_a100-dev|qos_gpu_a100-t3) ;;
  *) echo "Unsupported A100 QoS: $qos" >&2; exit 2 ;;
esac

export GNBM_REPO_DIR="$repo_dir"
export GNBM_MECH_OUTPUT="$experiment"
export GNBM_H1_DIAGNOSTIC_OUTPUT="$output"

log_dir="$WORK/logs/graph-native-betti-matching/a100"
mkdir -p "$log_dir"
submission="$(sbatch \
  --chdir="$repo_dir" \
  --qos="$qos" \
  --time="$walltime" \
  --output="$log_dir/%x-%j.out" \
  --error="$log_dir/%x-%j.err" \
  "$repo_dir/cluster/jean_zay/h1_birth_gradient_diagnostic_a100.slurm")"

echo "$submission"
job_id="${submission##* }"
echo "Queue:  squeue -j $job_id"
echo "Output: $output"
echo "Log:    $log_dir/gnbm-h1-birth-grad-$job_id.out"
