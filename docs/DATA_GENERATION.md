...
## Boundary-corrected graph revision

To preserve an existing `new_patches` dataset while correcting its VTP graphs,
write a separate revision and reuse the unchanged raw/segmentation files as
hard links:
...
The generator validates the old dataset's split, grid, patch size, padding,
stride, and normalization before creating any hard links. Hard links share the
unchanged file payload but remain valid if the old directory entry is later
