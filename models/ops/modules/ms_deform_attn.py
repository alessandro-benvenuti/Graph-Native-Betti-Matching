...
class MSDeformAttn(nn.Module):
    def __init__(
        ...
        self.use_cuda_extension = bool(use_cuda_extension)
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        :param input_spatial_shapes        (n_levels, 3)
        ...
        if reference_points.shape[-1] == 2:
            ...
            raise ValueError(
                "Last dimension of reference_points must be 2 or 3, "
        ...
        if self.use_cuda_extension:
            if not value.is_cuda:
                raise RuntimeError(
                    "The compiled deformable-attention path requires CUDA tensors; "
                    "set model.decoder.use_cuda_extension=false for CPU tests."
                )
            if not cuda_extension_available():
                raise RuntimeError(
                    "MultiScaleDeformableAttention3D is not installed. Build "
                    "models/ops before running the CUDA model."
                )
