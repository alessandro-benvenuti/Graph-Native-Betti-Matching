"""Checkpoint-compatible 3D deformable transformer."""
...
class DeformableTransformer(nn.Module):
    def __init__(
        ...
        encoder_layer = DeformableTransformerEncoderLayer(
            ...
            nhead, enc_n_points, use_cuda_extension
        )
...
class DeformableTransformerEncoder(nn.Module):
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for lvl, (D_, H_, W_) in enumerate(spatial_shapes):
            ...
            try:
                ref_z, ref_y, ref_x = torch.meshgrid(*coordinates, indexing="ij")
            except TypeError:
                # PyTorch versions predating the `indexing` keyword already use
                # matrix (`ij`) indexing, so this is numerically identical.
                ref_z, ref_y, ref_x = torch.meshgrid(*coordinates)
...
def build_def_detr_transformer(config: Mapping) -> DeformableTransformer:
    decoder = config["model"]["decoder"]
    return DeformableTransformer(
        d_model=int(decoder["hidden_dim"]),
        nhead=int(decoder["attention_heads"]),
        num_encoder_layers=int(decoder["encoder_layers"]),
        num_decoder_layers=int(decoder["decoder_layers"]),
        dim_feedforward=int(decoder["feedforward_dim"]),
        dropout=float(decoder["dropout"]),
        activation=str(decoder["activation"]),
        return_intermediate_dec=False,
        num_feature_levels=int(decoder["feature_levels"]),
        dec_n_points=int(decoder["decoder_points"]),
        enc_n_points=int(decoder["encoder_points"]),
        rln_attn=bool(decoder["relation_attention"]),
        use_cuda_extension=bool(decoder["use_cuda_extension"]),
    )
