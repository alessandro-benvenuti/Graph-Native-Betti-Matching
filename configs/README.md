...
The future loader must recursively merge the files listed in `defaults`, from
left to right, and then validate the resolved configuration before importing
datasets or constructing the model.

## Files

- `base.yaml`: shared model, optimizer, loss, augmentation, evaluation, and
  topology defaults.
- `pretrain_mixed.yaml`: trusted Plants + syntheticMRI mixed pretraining.
- `finetune_synthetic_mri.yaml`: trusted syntheticMRI-only finetuning with
...
The loader must report an unset variable as a configuration error.
...
The legacy loss also truncated positive edges to 40 when sampling mode was
...
Validation rules for the future configuration loader:
...
The baseline numerical values come from the trusted legacy configurations.
